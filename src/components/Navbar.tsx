...
export default function Navbar() {
  ...
  const handleNavClick = (e, href) => {
    // Jika user di halaman Katalog dan klik link section (bukan ke katalog), paksa pindah halaman
    if (isCatalogPage && !href.includes('#katalog')) {
      window.location.href = href.replace('/', '');
      return;
    }
    
    // Jika user di halaman utama, cegah reload dan lakukan scroll halus
    e.preventDefault();
  ...
  return (
    <nav className="fixed w-full z-50 top-3 px-4 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-6 rounded-full bg-brand-beige/95 backdrop-blur-md shadow-md border border-brand-brown-dark/5 py-2.5">
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer" className="bg-brand-green-leaf text-white px-6 py-2.5 rounded-full text-[15px] font-semibold hover:bg-brand-green-soft transition-all shadow-sm flex items-center gap-2">
      ...
      {/* Mobile Menu Dropdown tetap sama seperti kode sebelumnya */}
    </nav>
  );
}
