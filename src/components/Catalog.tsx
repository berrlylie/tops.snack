...
export default function Catalog({ currentPath }: { currentPath: string }) {
  ...
  // Menggunakan useMemo agar pemrosesan data hanya terjadi jika dbProducts berubah
  const allProducts = useMemo(() => {
    return [...PRODUCTS, ...dbProducts].map((p: any) => ({
      ...
      // Tambahkan properti ini di database Anda agar tidak perlu hardcode nama produk
                ...
                {categoryProducts.length > 0 ? (
                  categoryProducts.map((product, pIndex) => (
                    <motion.div
                      ...
                      className="w-full bg-white rounded-2xl overflow-hidden border border-brand-beige hover:border-brand-green-leaf/30 transition-all"
                    >
                      {/* Bagian Gambar */}
